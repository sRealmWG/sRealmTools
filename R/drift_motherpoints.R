#' Adds a fixed drift movement to mother point x coordinates
#'
...
drift_x_motherpoints <- function(mpcoords, drift) {
   ...
   xmother <- base::unlist(xmother) + drift
...
#' Adds a fixed drift movement to mother point y coordinates
...
drift_y_motherpoints <- function(mpcoords, drift) {
   ...
   ymother <- base::unlist(ymother) + drift
